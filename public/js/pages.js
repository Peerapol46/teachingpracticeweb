...
window.Pages = (function () {
  var base = {
    ...
    methods: {
      ...
      thaiDate: function (v, s) {
        return Store.thaiDate(v, s);
      },
      confirmDelete: function (msg) {
        return window.confirm(msg || "ต้องการลบรายการนี้ใช่หรือไม่?");
      },
    },
  };
  ...
  var btn = {
    ...
    ghost:
      "inline-flex items-center gap-2 rounded-xl border border-ink-200 bg-white px-4 py-2.5 text-sm font-medium text-ink-700 transition hover:border-brand-300 hover:text-brand-700",
    danger:
      "inline-flex items-center gap-2 rounded-xl border border-red-200 bg-white px-3 py-2 text-sm font-medium text-red-600 transition hover:bg-red-50",
  };
  ...
  var PROFILE_FIELDS = [
    {
      key: "reportTitle",
      label: "ชื่อรายงาน",
      placeholder: "รายงานการฝึกประสบการณ์วิชาชีพครู",
      wide: true,
    },
    {
      key: "studentName",
    ...
    {
      key: "period",
      label: "ช่วงเวลาฝึกสอน",
      placeholder: "16 พ.ค. 2568 – 30 ก.ย. 2568",
    },
  ];
  ...
  var Home = {
    ...
    computed: {
      ...
      filled: function () {
        ...
        return (
          PROFILE_FIELDS.some(function (f) {
            return f.key !== "reportTitle" && p[f.key];
          }) ||
          !!p.photo ||
          !!p.mentorPhoto
        );
      ...
      stats: function () {
        ...
        return [
          ...
          {
            label: "สัปดาห์ที่บันทึก",
            value: Object.keys(weeks).length,
            unit: "สัปดาห์",
            icon: "calendar",
            page: "log",
          },
      ...
      cover: function () {
        var d = this.d;
        return (
          (d.profile.photo && (d.profile.photo.thumb || d.profile.photo.url)) ||
          (d.school.logo && (d.school.logo.thumb || d.school.logo.url)) ||
          null
        );
      },
    },
    template: [
      ...
      '      <div v-if="cover" class="shrink-0 w-24 h-24 sm:w-28 sm:h-28 rounded-3xl overflow-hidden ring-4 ring-white/30 bg-white/20">',
      '        <img :src="cover" class="w-full h-full object-cover" referrerpolicy="no-referrer" alt=""></div>',
      '      <div class="min-w-0">',
      ...
      "      <template #actions>",
      '        <button v-if="edit && filled" class="' +
        btn.ghost +
        ' !py-2" @click="openEdit"><ui-icon name="edit" cls="w-4 h-4"/>แก้ไขข้อมูล</button>',
      "      </template>",
      '      <ui-empty v-if="!filled" icon="edit" text="ยังไม่ได้กรอกข้อมูลผู้ฝึกสอน">',
      '        <button v-if="edit" class="' +
        btn.primary +
        '" @click="openEdit"><ui-icon name="plus" cls="w-4 h-4"/>กรอกข้อมูลผู้ฝึกสอน</button>',
      "      </ui-empty>",
      ...
      '      <button class="mt-4 w-full ' +
        btn.ghost +
        " justify-center\" @click=\"$emit('go', 'log')\">ดูบันทึกทั้งหมด</button>",
      "    </ui-card>",
      "  </div>",
  ...
  var SCHOOL_FIELDS = [
    { key: "name", label: "ชื่อสถานศึกษา" },
    {
      key: "affiliation",
      label: "สังกัด",
      placeholder: "เช่น สพป. / สพม. / อปท.",
    },
    { key: "director", label: "ผู้อำนวยการ" },
    {
      key: "size",
      label: "ขนาดสถานศึกษา",
      placeholder: "เช่น ขนาดกลาง / นักเรียน 480 คน",
    },
  ...
  var School = {
    ...
    computed: {
      ...
      filled: function () {
        var s = this.d.school;
        return SCHOOL_FIELDS.some(function (f) {
          return s[f.key];
        });
      },
    },
    methods: {
      openEdit: function () {
        var s = this.d.school,
          form = {};
        SCHOOL_FIELDS.forEach(function (f) {
          form[f.key] = s[f.key] || "";
        });
        this.form = form;
        this.show = true;
    ...
    template: [
      ...
      '        <button v-if="edit && filled" class="' +
        btn.ghost +
        ' !py-2" @click="openEdit"><ui-icon name="edit" cls="w-4 h-4"/>แก้ไข</button>',
      "      </template>",
      '      <ui-empty v-if="!filled" icon="edit" text="ยังไม่ได้กรอกข้อมูลสถานศึกษา">',
      '        <button v-if="edit" class="' +
        btn.primary +
        '" @click="openEdit"><ui-icon name="plus" cls="w-4 h-4"/>กรอกข้อมูลสถานศึกษา</button>',
      "      </ui-empty>",
  ...
  var Log = {
    ...
    computed: {
      dayOptions: function () {
        return Store.WORK_DAYS.map(function (i) {
          return { value: i, label: "วัน" + Store.THAI_DAYS[i] };
        });
      },
      weekList: function () {
        var set = {};
      ...
      groups: function () {
        ...
        return Object.keys(map)
          ...
          .filter(function (w) {
            return self.filterWeek === "all" || self.filterWeek === w;
          })
          .map(function (w) {
            var items = map[w];
            ...
            return {
              ...
              days: Store.WORK_DAYS.map(function (di) {
                ...
                var dated = cell.filter(function (l) {
                  return l.date;
                })[0];
                return {
                  index: di,
      ...
      sorted: function () {
        return this.d.logs.slice().sort(function (a, b) {
          return (
            ...
            String(a.period || "").localeCompare(String(b.period || ""), "th", {
              numeric: true,
            })
          );
        });
    ...
    methods: {
      ...
      rangeOf: function (items) {
        ...
        return dates[0] === dates[dates.length - 1]
          ...
          : Store.thaiDate(dates[0], "short") +
              " – " +
              Store.thaiDate(dates[dates.length - 1], "short");
      },
        ...
        this.form.week =
          week && week !== "-"
            ...
            : this.weekList
                ...
                .pop() || "1";
        if (day) this.form.day = Number(day);
      ...
      onDateChange: function () {
        var di = Store.dayIndex(this.form.date);
        if (di === null) return;
        if (di === 0 || di === 6) {
          Store.toast(
            "ตารางบันทึกมีเฉพาะวันจันทร์–ศุกร์ (ไม่รวมวันหยุดเสาร์–อาทิตย์)",
            "error",
          );
          this.form.date = "";
      ...
      submit: function () {
        ...
        if (this.editingId) {
          ...
          var i = this.d.logs.findIndex(function (l) {
            return l.id === idToFind;
          });
          if (i > -1) {
            f.id = idToFind;
            this.d.logs.splice(i, 1, f);
          }
        } else {
          f.id = this.uid();
          this.d.logs.push(f);
      ...
      remove: function (item) {
        var title =
          item.topic || Store.thaiDate(item.date, "short") || "รายการนี้";
        if (!this.confirmDelete('ลบบันทึก "' + title + '" ?')) return;
        var i = this.d.logs.findIndex(function (l) {
          return l.id === item.id;
        });
    ...
    template: [
      ...
      '      <button v-if="edit" class="' +
        btn.primary +
        '" @click="openNew()"><ui-icon name="plus" cls="w-4 h-4"/>เพิ่มบันทึก</button>',
      "    </template>",
      "  </page-head>",
      ...
      '    <button v-if="edit" class="' +
        btn.primary +
        '" @click="openNew(\'1\')"><ui-icon name="plus" cls="w-4 h-4"/>เพิ่มบันทึกแรก</button>',
      "  </ui-empty>",
      ...
      '        <button v-if="edit" class="' +
        btn.ghost +
        ' !py-1.5 !px-3" @click="openNew(g.week)"><ui-icon name="plus" cls="w-4 h-4"/>เพิ่ม</button>',
      "      </template>",
  ...
  var Plan = {
    ...
    methods: {
      ...
      openFile: function (file) {
        ...
        var url = file.url || file.download || file.thumb;

        if (url) {
          window.open(url, "_blank");
        } else {
          Store.toast("ไม่พบไฟล์สำหรับเปิดดู", "error");
        }
      },
      ...
      submit: function () {
        if (!this.form.name) {
          Store.toast("กรุณากรอกชื่อแผนการสอน", "error");
        ...
        if (this.editingId) {
          ...
          var i = this.d.plans.findIndex(function (p) {
      ...
      remove: function (p) {
        if (!this.confirmDelete('ลบแผนการสอน "' + (p.name || "") + '" ?'))
          return;

        var i = this.d.plans.findIndex(function (x) {
          return x.id === p.id;
        });

        if (i > -1) {
          this.d.plans.splice(i, 1);
        }
    ...
    template: [
      ...
      "    <template #actions>",
      '      <button v-if="edit" class="' + btn.primary + '" @click="openNew">',
      '        <ui-icon name="plus" cls="w-4 h-4"/>เพิ่มแผนการสอน',
      "      </button>",
      "    </template>",
      "  </page-head>",
      ...
      '    <button v-if="edit" class="' + btn.primary + '" @click="openNew">',
      '      <ui-icon name="plus" cls="w-4 h-4"/>เพิ่มแผนแรก',
      "    </button>",
      "  </ui-empty>",
      ...
      "      <template #actions>",
      '        <button v-if="edit" @click="openEdit(p)" class="text-ink-400 hover:text-brand-600 p-1">',
      '          <ui-icon name="edit" cls="w-4 h-4"/>',
      "        </button>",
      "      </template>",

      '      <p v-if="p.detail" class="text-sm text-ink-600 whitespace-pre-line leading-relaxed">',
      "        {{ p.detail }}",
      "      </p>",
      ...
      '          <p class="text-xs font-medium text-ink-500 mb-1.5">',
      "            ไฟล์สไลด์",
      "          </p>",

      '          <div v-if="p.slide" class="rounded-2xl transition duration-300 hover:-translate-y-1 hover:shadow-card hover:border-brand-300">',

      "            <ui-asset",
      '              :asset="p.slide"',
      '              :removable="edit"',
      '              @click="openFile(p.slide)"',
      '              @remove="p.slide=null; save()"/>',

      "          </div>",

      '          <p v-else class="text-xs text-ink-300 py-2">',
      "            ยังไม่มีไฟล์",
      "          </p>",
      ...
      '          <div v-if="p.file" class="rounded-2xl transition duration-300 hover:-translate-y-1 hover:shadow-card hover:border-brand-300">',

      "            <ui-asset",
      '              :asset="p.file"',
      '              :removable="edit"',
      '              @click="openFile(p.file)"',
      '              @remove="p.file=null; save()"/>',
      ...
      '        <ui-field label="ชื่อแผน" v-model="form.name" placeholder="เช่น แผนการจัดการเรียนรู้ที่ 1 เรื่อง จำนวนนับ"/>',
      "      </div>",
      ...
      '      <div class="sm.col-span-2">',
      '        <ui-field label="รายละเอียด" type="textarea" :rows="5" v-model="form.detail"/>',
      "      </div>",
      ...
      '        <div v-if="form.slide" class="mb-2">',
      '          <ui-asset :asset="form.slide" removable @remove="form.slide=null"/>',
      "        </div>",
      ...
      '        <div v-if="form.file" class="mb-2">',
      '          <ui-asset :asset="form.file" removable @remove="form.file=null"/>',
      "        </div>",
      ...
      '      <button v-if="editingId" class="' +
        btn.danger +
        ' mr-auto" @click="remove(form)">',
      '        <ui-icon name="trash" cls="w-4 h-4"/>ลบ',
      "      </button>",

      '      <button class="' + btn.ghost + '" @click="show=false">',
      "        ยกเลิก",
      "      </button>",
  ...
  var Research = {
    ...
    data: function () {
      return {
        show: false,
        form: { id: "", title: "", detail: "", files: [] },
        editingId: null,
      };
    },
    methods: {
      openNew: function () {
        this.form = { id: "", title: "", detail: "", files: [] };
        this.editingId = null;
        this.show = true;
      },
      openEdit: function (r) {
      ...
      submit: function () {
        if (!this.form.title) {
          Store.toast("กรุณากรอกชื่อวิจัยในชั้นเรียน", "error");
          return;
        }
        var f = JSON.parse(JSON.stringify(this.form));
        if (this.editingId) {
          var idToFind = this.editingId;
          var i = this.d.research.findIndex(function (r) {
            return r.id === idToFind;
          });
          if (i > -1) {
            f.id = idToFind;
            this.d.research.splice(i, 1, f);
          }
        } else {
          f.id = this.uid();
          this.d.research.push(f);
        }
        this.save();
        this.show = false;
      },
      remove: function (r) {
        if (!this.confirmDelete('ลบงานวิจัย "' + (r.title || "") + '" ?'))
          return;
        var i = this.d.research.findIndex(function (x) {
          return x.id === r.id;
        });
    ...
    template: [
      ...
      '    <template #actions><button v-if="edit" class="' +
        btn.primary +
        '" @click="openNew"><ui-icon name="plus" cls="w-4 h-4"/>เพิ่มงานวิจัย</button></template>',
      "  </page-head>",
      '  <ui-empty v-if="!d.research.length" icon="research" text="ยังไม่มีงานวิจัยในชั้นเรียน">',
      '    <button v-if="edit" class="' +
        btn.primary +
        '" @click="openNew"><ui-icon name="plus" cls="w-4 h-4"/>เพิ่มงานวิจัย</button>',
      "  </ui-empty>",
      '  <div v-else class="space-y-5">',
  ...
  var Gallery = {
    ...
    computed: {
      items: function () {
        return this.d[this.field];
      },
    },

    methods: {
      openLightbox: function (p) {
        this.lightbox = p;
      },
    ...
    template: [
      ...
      "    <template #actions>",
      '      <file-drop v-if="edit" compact multiple accept="image/*" label="อัปโหลดรูป" @uploaded="add"/>',
      "    </template>",
      ...
      '        <div class="relative aspect-[4/3] bg-ink-50 cursor-zoom-in" @click="openLightbox(p)">',

      "          <img",
      '            :src="p.file.thumb || p.file.url"',
      ...
      '          <button v-if="edit"',
      '            @click.stop="remove(i)"',
      '            class="no-print absolute top-2 right-2 w-8 h-8 rounded-xl bg-white/95 text-ink-600 shadow-soft opacity-0 group-hover:opacity-100 transition flex items-center justify-center hover:text-red-600">',
      ...
      '             class="w-full text-sm bg-transparent outline-none placeholder:text-ink-300 border-b border-transparent focus:border-brand-300 pb-1">',
      ...
      '             type="date"',
      '             v-model="p.date"',
      '             class="w-full text-xs text-ink-500 bg-transparent outline-none">',

      '          <p v-else-if="p.date" class="text-xs text-ink-400">',
      '             {{ thaiDate(p.date, "short") }}',
      "          </p>",
      ...
      '      <file-drop multiple accept="image/*" label="เพิ่มรูปอีก" hint="ลากไฟล์มาวางได้" @uploaded="add"/>',
      "    </div>",
      ...
      '  <div v-if="lightbox"',
      '       class="fixed inset-0 z-[999] bg-black/85 backdrop-blur-sm flex items-center justify-center p-5 no-print"',
      '       @click="closeLightbox">',

      "      <button",
      '        @click.stop="closeLightbox"',
      '        class="absolute top-5 right-5 w-12 h-12 rounded-full bg-white/90 text-gray-700 text-3xl shadow-xl flex items-center justify-center hover:bg-white">',
      "        ×",
      "      </button>",

      '      <figure class="max-w-6xl w-full flex flex-col items-center" @click.stop>',

      "        <img",
      '          :src="lightbox.file.thumb || lightbox.file.url"',
      '          class="max-w-[95vw] max-h-[85vh] object-contain rounded-2xl shadow-2xl"',
      '          referrerpolicy="no-referrer"',
      '          alt="">',

      '        <figcaption v-if="lightbox.caption || lightbox.date"',
      '          class="text-center text-white/90 text-sm mt-4">',

      "          {{ lightbox.caption }}",

      '          <span v-if="lightbox.date" class="text-white/60">',
      '             · {{ thaiDate(lightbox.date, "short") }}',
      "          </span>",
  ...
  var Summary = {
    ...
    computed: {
      ...
      counts: function () {
        ...
        return [
          ...
          ["นิเทศการสอน", (d.supervisions || []).length + " ครั้ง"],
    ...
    template: [
      ...
      '    <template #actions><button class="' +
        btn.ghost +
        '" @click="print"><ui-icon name="print" cls="w-4 h-4"/>พิมพ์ / บันทึก PDF</button></template>',
      "  </page-head>",
      '  <div class="grid lg:grid-cols-3 gap-5">',
  ...
  var Settings = {
    ...
    data: function () {
      var saved = {};
      try {
        saved = JSON.parse(localStorage.getItem("tp.config") || "{}");
      } catch (e) {
        /* ignore */
      }
      return {
        clientId:
          saved.googleClientId || window.APP_CONFIG.googleClientId || "",
    ...
    methods: {
      ...
      saveConfig: function () {
        ...
        if (id && !/\.apps\.googleusercontent\.com$/.test(id)) {
          Store.toast(
            "Client ID ไม่ถูกต้อง — ต้องลงท้ายด้วย .apps.googleusercontent.com",
            "error",
          );
          return;
        }
        if (key && !/^AIza[0-9A-Za-z_-]{20,}$/.test(key)) {
          Store.toast(
            "API key ไม่ถูกต้อง — ต้องเป็นค่าที่ขึ้นต้นด้วย AIza ไม่ใช่ลิงก์หรือค่าอื่น",
            "error",
          );
          return;
        }
        ...
        localStorage.setItem(
          "tp.config",
          JSON.stringify({ googleClientId: id, googleApiKey: key }),
        );
      ...
      clearConfig: function () {
        if (
          !window.confirm(
            "ลบค่าเชื่อมต่อที่บันทึกไว้ในเครื่องนี้ แล้วกลับไปใช้ค่าจากไฟล์ config.js?",
          )
        )
          return;
      ...
      importJson: function (e) {
        ...
        if (f)
          Store.importJson(f).catch(function (err) {
      ...
      resetAll: function () {
        if (
          !window.confirm(
            "ล้างข้อมูลทั้งหมดในเครื่องนี้? (ไฟล์ที่อยู่ใน Google Drive จะไม่ถูกลบ)",
          )
        )
          return;
    ...
    template: [
      ...
      '        <button class="' +
        btn.ghost +
        '" @click="signOut"><ui-icon name="logout" cls="w-4 h-4"/>ออกจากระบบ</button>',
      "      </div>",
      /* ยังไม่ล็อกอิน */
      "      <div v-else>",
      ...
      '        <button class="' +
        btn.primary +
        '" @click="signIn" :disabled="s.signingIn">',
      ...
      "      </div>",
      '      <a v-if="folderUrl" :href="folderUrl" target="_blank" rel="noopener" class="' +
        btn.ghost +
        ' mt-4">',
      ...
      '        <button class="' +
        btn.ghost +
        '" @click="exportJson"><ui-icon name="download" cls="w-4 h-4"/>ดาวน์โหลดไฟล์ข้อมูล</button>',
      '        <label class="' +
        btn.ghost +
        ' cursor-pointer"><ui-icon name="upload" cls="w-4 h-4"/>นำเข้าไฟล์ข้อมูล',
      ...
      '      <button class="' +
        btn.danger +
        '" @click="resetAll"><ui-icon name="trash" cls="w-4 h-4"/>ล้างข้อมูลในเครื่องนี้</button>',
      "    </ui-card>",
      "  </div>",
      ...
      '        <button class="' +
        btn.primary +
        '" @click="saveConfig"><ui-icon name="check" cls="w-4 h-4"/>บันทึกค่าเชื่อมต่อ</button>',
      '        <button v-if="configSource === \'local\'" class="' +
        btn.ghost +
        '" @click="clearConfig"><ui-icon name="trash" cls="w-4 h-4"/>ล้างค่าที่กรอกในเครื่องนี้</button>',
      "      </div>",
      "    </div>",
      "  </section>",
      "</div>",
