  ...
  window.Pages = (function () {
    ...
    var Home = {
      ...
      computed: {
        ...
        cover: function () {
          var d = this.d;
          return (d.profile.photo && d.profile.photo.thumb) || (d.school.logo && d.school.logo.thumb) || null;
        }
      },
      template: [
        '<div>',
...
'    </div>',
    ...
    var Schedule = {
      mixins: [base],
      data: function () { return { term: 0, noteShow: false, noteForm: '' }; },
      computed: {
        current: function () { return this.d.schedules[this.term] || this.d.schedules[0]; }
      },
      methods: {
        openNote: function () { this.noteForm = this.current.note || ''; this.noteShow = true; },
        submitNote: function () { this.current.note = this.noteForm; this.save(); this.noteShow = false; }
      },
      template: [
        '<div>',
        '  <page-head icon="calendar" title="ตารางสอน" subtitle="อัปโหลดรูปตารางสอน แยกตามภาคเรียน">',
        '    <template #actions>',
        '      <button v-if="edit" class="' + btn.ghost + '" @click="openNote"><ui-icon name="edit" cls="w-4 h-4"/>แก้ไขหมายเหตุ</button>',
        '    </template>',
        '  </page-head>',
        ...
        '      {{ t.label }}<span class="ml-1.5 opacity-70">({{ t.images.length }})</span></button>',
        '  </div>',

        '  <ui-card v-for="(t,i) in d.schedules" :key="t.id" v-show="term===i" :title="t.label">',
        '    <p v-if="t.note" class="text-sm text-ink-600 whitespace-pre-line leading-relaxed mb-5 pb-5 border-b border-ink-100">{{ t.note }}</p>',
        '    <div v-if="t.images.length" class="grid sm:grid-cols-2 gap-4 mb-5">',
        '      <ui-asset v-for="(img,j) in t.images" :key="img.id" :asset="img" :removable="edit" ratio="aspect-[16/9]" @remove="t.images.splice(j,1); save()"/>',
        '    </div>',
        '    <file-drop v-if="edit" multiple accept="image/*,application/pdf" :label="\'อัปโหลดรูปตารางสอน \' + t.label"',
        '      hint="ลากไฟล์มาวางได้ · รองรับหลายไฟล์" @uploaded="t.images.push(...$event); save()"/>',
        '    <ui-empty v-else-if="!t.images.length" icon="calendar" text="ยังไม่มีตารางสอนของภาคเรียนนี้"/>',
        '  </ui-card>',
        ...
        '    <template #footer>',
        '      <button class="' + btn.ghost + '" @click="noteShow=false">ยกเลิก</button>',
        '      <button class="' + btn.primary + '" @click="submitNote"><ui-icon name="check" cls="w-4 h-4"/>บันทึก</button>',
        '    </template>',
        '  </ui-modal>',
        '</div>'
      ].join('')
    ...
    var Log = {
      ...
      template: [
        ...
        '              </article>',
        '              <button v-if="edit" @click="openNew(g.week, col.index)"',
    ...
    var Gallery = {
      ...
      template: [
        ...
        '  <transition name="fade"><div v-if="lightbox" class="fixed inset-0 z-[60] bg-ink-900/85 flex items-center justify-center p-4 no-print" @click="lightbox=null">',
        '    <figure class="max-w-5xl w-full">',
        '      <img :src="lightbox.file.url || lightbox.file.thumb" class="w-full max-h-[80vh] object-contain rounded-2xl" referrerpolicy="no-referrer" alt="">',
        '      <figcaption v-if="lightbox.caption || lightbox.date" class="text-center text-white/90 text-sm mt-3">',
        '        {{ lightbox.caption }}<span v-if="lightbox.date" class="text-white/60"> · {{ thaiDate(lightbox.date, "short") }}</span></figcaption>',
        '    </figure>',
        '  </div></transition>',
        '</div>'
      ].join('')
    ...
    return { Home: Home, School: School, Schedule: Schedule, Log: Log, Plan: Plan, Research: Research, Gallery: Gallery, Summary: Summary, Settings: Settings };
  })();
